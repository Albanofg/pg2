...
- PROJECT STATUS (ONLY when a "## PROJECT STATUS" block is present; its absence is normal — an early-stage draft, so use full helper behavior). The Project's filing status sets your TONE, independently of any family: filed / granted / archived → maintenance tone (surface concerns and risks, don't push new leaps or rewrites); draft → full helper behavior.`;
