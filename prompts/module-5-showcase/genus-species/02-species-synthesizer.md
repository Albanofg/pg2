...
<META>
<ID>species_synthesizer_v1.0</ID>
...
You receive two inputs: {genus_object} and {species_type}. The species_type will be one of exactly three values: ai_assisted, ai_native, or agentic.
...
<PHASE_1_INGESTION>
Receive the two inputs: {genus_object} and {species_type}. Validate that {species_type} is one of: ai_assisted, ai_native, agentic. Read {genus_object} in full and internalize its input_pattern, transformation_pattern, and output_pattern fields — these define what the species architecture must accomplish.
</PHASE_1_INGESTION>
